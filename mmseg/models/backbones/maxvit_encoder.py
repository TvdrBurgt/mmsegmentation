# github: https://github.com/ChristophReich1996/MaxViT
...
class MaxViTBlock(nn.Module):
    ...
    def __init__(
        ...
        if self.debug: print(self.module_name, '[downscale]', downscale)
...
@BACKBONES.register_module(name='MaxViT', force=True)
class MaxViT(nn.Module):
    ...
    def __init__(
        ...
        self.stem = nn.Sequential(
            nn.Conv2d(in_channels=in_channels, out_channels=embed_dim, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1)),
            act_layer(),
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        outs = []
        x = self.stem(input)
        if self.debug: print('[MaxViT][forward]', '[stem][x]', x.shape)
...
if __name__ == '__main__':
    ...
    def test_block() -> None:
        block = MaxViTBlock(in_channels=128, out_channels=256, downscale=True, grid_window_size=(8, 8))
        ...
        input = torch.rand(1, 128, 32, 32)
        output = block(input)
        print(output.shape)

        input = torch.rand(1, 128, 16, 16)
        output = block(input)
        print(output.shape)
    ...
    def test_networks() -> None:
        for get_network in [max_vit_tiny_224, max_vit_small_224, max_vit_base_224, max_vit_large_224]:
            ...
            print(output.shape)
            print(get_network, sum(param.numel() for param in network.parameters()) / 1e6)
    ...
    output = model(input)
    print(output.shape)
    print(get_relative_position_index(4, 4))
